...
from .vdeployment_inputs import VDeploymentInputGenerator
...
def test_normal(
    ...
    namespace = deploy.operator_existing_namespace or CONST.ACTO_NAMESPACE
    ...
    # run the workload
    # Built-in controllers (e.g. Deployment) have no CRD; skip process_crd.
    if config.crd_name:
        crd = process_crd(
            kubernetes_client(kubeconfig, context_name),
            KubectlClient(kubeconfig, context_name),
            crd_name=config.crd_name,
        )
    else:
        crd = {}
    ...
    if "normal" in modes:
        ...
        for workload in workloads:
            measurement_result = runner.measure(
                workload,
                sts_name_f,
                ds_name_f,
                gen,
...
def main(args):
    ...
    if "reference" in args.phase:
        with open(args.reference_config, "r", encoding="UTF-8") as config_file:
            config = json.load(config_file)
            if "monkey_patch" in config:
                del config["monkey_patch"]
            config = OperatorConfig(**config)
        ...
        test_normal(
            ...
            modes=args.modes,
            sample_rate=args.sample,
            deployment_name_f=reference_deployment_name_f,
        )
