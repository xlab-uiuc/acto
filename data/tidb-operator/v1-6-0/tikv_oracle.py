...
class TiKVConfigChecker(CheckerInterface):
    """Custom oracle for checking TiKV config"""

    name = "tikv-config-checker"
    ...
    def __check_config(
        ...
        """Check the TiKV config"""
        _, _, _ = generation, snapshot, prev_snapshot

        logger = get_thread_logger()
        logger.info("Checking TiKV config")
        if (
            "tikv" in snapshot.input_cr["spec"]
            and "config" in snapshot.input_cr["spec"]["tikv"]
        ):
            tikv_spec_config = snapshot.input_cr["spec"]["tikv"]["config"]
        else:
            return None

        tikv_config = tomlkit.loads(tikv_spec_config)
        ...
        if p.returncode != 0:
            return OracleResult(message="TiKV config check failed")
        ...
        for node, value in all_node_config.items():
            if not compare_methods.equals(tikv_config, value):
                return OracleResult(
                    message="TiKV config check failed due to "
...
CUSTOM_CHECKER: type[CheckerInterface] = TiKVConfigChecker
