...
def check_state_equality(snapshot: Snapshot, prev_snapshot: Snapshot) -> OracleResult:
    ...
    if diff:
        logger.debug(
            f"failed attempt recovering to seed state - system state diff: {diff}"
        )
...
class TrialRunner:
    ...
    def run(self, errors: List[RunResult], mode: str = InputModel.NORMAL):
        ...
        while True:
            ...
            self.cluster.restart_cluster(
                self.cluster_name, self.kubeconfig, CONST.K8S_VERSION
            )
            ...
            trial_dir = os.path.join(
                self.workdir,
                "trial-%02d-%04d"
                % (self.worker_id + self.sequence_base, self.curr_trial),
            )
            ...
            logger.info(
                "Trial %d finished, completed in %s" % (self.curr_trial, trial_elapsed)
            )
            logger.info(
                f"Kubernetes bootstrap: {trial_k8s_bootstrap_time - trial_start_time}"
            )
            logger.info(
                f"Operator deploy: {operator_deploy_time - trial_k8s_bootstrap_time}"
            )
    ...
    def run_trial(
        ...
        runner: Runner = self.runner_t(
            self.context, trial_dir, self.kubeconfig, self.context_name, self.wait_time
        )
        checker: CheckerSet = self.checker_t(
            self.context, trial_dir, self.input_model, oracle_handle, self.custom_oracle
        )
        ...
        while (
            ...
            if generation > 0:
                ...
                for (
                    ...
                    if testcase.test_precondition(field_curr_value):
                        ...
                        logger.info(
                            "Precondition of %s fails, try setup first", field_path_str
                        )
                        ...
                        runResult = TrialRunner.run_and_check(
                            runner,
                            checker,
            ...
            t = self.run_testcases(
                curr_input_with_schema, ready_testcases, runner, checker, generation
            )
            ...
            if len(testcase_patches) == 1:
                ...
                if responsible_field == None:
                    ...
                    for group, testcase_with_path, patch in testcase_patches:
                        ...
                        if (
                            not iso_result.is_invalid()[0] and iso_result.is_error()
    ...
    def run_recovery(
        self, runner: Runner, checker: CheckerSet, generation: int
    ) -> OracleResult:
    ...
    def revert(self, runner, checker, generation) -> ValueWithSchema:
        ...
        result = TrialRunner.run_and_check(
            runner,
            checker,
            curr_input_with_schema.raw_value(),
            self.snapshots,
...
class Acto:
    ...
    def __init__(
        ...
        if operator_config.deploy.method == "HELM":
            deploy = Deploy(
                DeployMethod.HELM,
                operator_config.deploy.file,
                operator_config.deploy.init,
            ).new()
        ...
        if operator_config.custom_fields != None:
            ...
            for tuple in tuples:
                custom_field = OverSpecifiedField(
                    tuple[0].path, array=isinstance(tuple[1], ArrayGenerator)
                )
                self.input_model.apply_custom_field(custom_field)
    ...
    def __learn(self, context_file, helper_crd, analysis_only=False):
        ...
        if os.path.exists(context_file):
            ...
            while True:
                self.cluster.restart_cluster(
                    "learn", learn_kubeconfig, CONST.K8S_VERSION
                )
                deployed = self.deploy.deploy_with_retry(
    ...
    def run(
        ...
        if InputModel.ADDITIONAL_SEMANTIC in modes:
            ...
            for runner in runners:
                t = threading.Thread(
                    target=runner.run, args=([errors, InputModel.ADDITIONAL_SEMANTIC])
                )
                t.start()
                threads.append(t)
        ...
        with open(
            os.path.join(self.workdir_path, "testrun_info.json"), "w"
        ) as info_file:
