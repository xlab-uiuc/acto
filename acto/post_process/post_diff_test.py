...
def compare_system_equality(
    ...
    for k, v in curr_pods.items():
        if (
            ...
            and v["metadata"]["owner_references"] != None
    ...
    if len(curr_system_state["secret"]) != len(prev_system_state["secret"]):
        ...
        return RecoveryResult(
            delta=DeepDiff(
                len(curr_system_state["secret"]), len(prev_system_state["secret"])
            ),
            from_=prev_system_state,
            to_=curr_system_state,
        )
...
class AdditionalRunner:
    ...
    def run_cr(self, cr, trial, gen):
        self._cluster.restart_cluster(
            self._cluster_name, self._kubeconfig, CONST.K8S_VERSION
        )
        apiclient = kubernetes_client(self._kubeconfig, self._context_name)
        deployed = self._deploy.deploy_with_retry(
            self._context, self._kubeconfig, self._context_name
        )
        ...
        difftest_result_path = os.path.join(
            trial_dir, "difftest-%03d.json" % self._generation
        )
...
class DeployRunner:
    ...
    def run(self):
        ...
        while True:
            ...
            difftest_result_path = os.path.join(
                trial_dir, "difftest-%03d.json" % generation
            )
            with open(difftest_result_path, "w") as f:
                json.dump(difftest_result, f, cls=ActoEncoder, indent=6)
...
class PostDiffTest(PostProcessor):
    ...
    def post_process(self, workdir: str, num_workers: int = 1):
        ...
        deploy = Deploy(
            DeployMethod.YAML, self.config.deploy.file, self.config.deploy.init
        ).new()
        ...
        while True:
            ...
            for original in originals:
                ...
                step_result = PostDiffTest.check_diff_test_step(
                    diff_test_result, original_result, self.config, False, runner
                )
    ...
    def check_diff_test_step(
        diff_test_result: Dict,
        original_result: Step,
        config: OperatorConfig,
        run_check_indeterministic: bool = False,
        additional_runner: AdditionalRunner = None,
    ) -> RecoveryResult:
        logger = get_thread_logger(with_prefix=True)
        ...
        if isinstance(result, PassResult):
            ...
            add_snapshot = additional_runner.run_cr(
                diff_test_result["snapshot"]["input"], trial_dir, gen
            )
            ...
            if len(indeterministic_fields) > 0:
                logger.info(
                    f"Got additional nondeterministic fields: {indeterministic_fields}"
                )
                for delta_category in result.delta:
                    for delta in result.delta[delta_category]:
                        if (
                            ...
                            logger.error(
                                f"Fail diff test for trial {trial_dir} gen {gen}"
                            )
