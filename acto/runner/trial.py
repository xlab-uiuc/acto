...
class Trial:
    ...
    def history_iterator(self) -> Generator[Tuple[Tuple[dict, dict], Union[Exception, Tuple[Snapshot, List[OracleResult]]]], None, None]:
        ...
        assert len(self.__snapshots) == len(self.__run_results)
        for (system_input, data) in zip_longest(system_input_iter, zip(self.__snapshots, self.__run_results)):
            if data is None:
                assert self.__error
                yield system_input, self.__error
            else:
                yield system_input, data
