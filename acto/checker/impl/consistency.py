...
def find_nearest_parent(
    path: PropertyPath, encoded_path_list: list[str]
    ...
    for encoded_path in encoded_path_list:
        ...
        if len(p) > length:
            ret = p
    return PropertyPath(ret) if ret is not None else None
...
def check_field_dependencies_are_satisfied(
    ...
    encoded_path = str(input_delta.path)
    ...
    if encoded_path in field_conditions_map:
        ...
        if parent is not None:
            condition_group = field_conditions_map[str(parent)]
    if condition_group and not check_condition_group(
        snapshot.input_cr, condition_group, input_delta.path
    ):
...
def check_field_has_default_value(
    ...
    try:
        default_value = input_model.get_schema_by_path(
            input_delta.path.path
        ).default
...
def list_matched_fields(
    k8s_paths: List[List[str]],
    path: list,
    delta_dict: dict[str, dict[str, dict[str, Diff]]],
...
class ConsistencyChecker(CheckerInterface):
    ...
    def check(
                    ...
                    if (
                        ...
                        if is_invalid:
                            ...
                            return InvalidInputResult(
                                message="Invalid input determined from status "
                                + f"message: {delta.curr}",
                                responsible_property=responsible_path,
                            )
        ...
        for diff_type, input_delta_list in input_delta.items():
            for input_diff in input_delta_list.values():
                logger.debug("Checking input delta [%s]", input_diff.path)

                # whether we expect the system to produce a delta
                must_produce_delta = True

                if "ephemeralContainers" in input_diff.path:
                    must_produce_delta = False

                if not check_field_dependencies_are_satisfied(
                    input_diff, snapshot, self.field_conditions_map
                ):
                    must_produce_delta = False
                ...
                if (
                    ...
                    input_diff.prev
                ) or is_none_or_not_present(input_diff.curr):
                    must_produce_delta = False

                if ACTO_CONFIG.checkers.state.enable_canonicalization:
                    canonicalized_prev = canonicalize_quantity(input_diff.prev)
                    canonicalized_curr = canonicalize_quantity(input_diff.curr)
                ...
                if not found and should_compare:
                            ...
                            for type_delta_list in resource_delta_list.values():
                                for state_delta in type_delta_list.values():
                                    if compare_method.equals_after_transform(
                                        ...
                                        found_state_delta = state_delta

                        if found_state_delta is not None:
                            logger.info(
                            ...
                            logger.info(
                                ...
                                found_state_delta.prev,
                                found_state_delta.curr,
                            )
                            continue
                        logger.error("Found no matching fields for input delta")
                        logger.error("Input delta [%s]", input_diff.path)
                        return ConsistencyOracleResult(
                            message="Found no matching fields for input",
                            input_diff=input_diff,
                            system_state_diff=None,
                        )
                    else:
                        logger.error("Found no matching fields for input delta")
                        logger.error("Input delta %s", input_diff.path)
                    ...
                    logger.debug("Input delta [%s] is skipped", input_diff.path)
