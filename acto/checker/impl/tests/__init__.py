...
def get_filename(
    ...
    return os.path.join(
        base_dir,
        f"{checker_name}_{file_type}_{test_case_id}{'_prev' if load_prev else ''}.{file_format}",
    )
...
def get_file(
    ...
    file_path = get_filename(
        checker_name, test_case_id, file_type, file_format, load_prev
    )
...
def load_operator_log(
    checker_name: str, test_case_id: int, load_prev=False
) -> List[str]:
...
def load_snapshot(
    ...
    return Snapshot(
        input_cr=load_input(checker_name, test_case_id, load_prev),
