...
from acto.common import is_subfield, translate_op


def check_condition(snapshot_input: dict, condition: dict, input_delta_path: list) -> bool:
    ...
    if 'type' in condition_group:
        ...
        if typ == 'AND':
            for condition in condition_group['conditions']:
                if not check_condition_group(snapshot_input, condition, input_delta_path):
                    return False
            return True
        elif typ == 'OR':
            for condition in condition_group['conditions']:
                if check_condition_group(snapshot_input, condition, input_delta_path):
                    return True
            return False
    else:
        return check_condition(snapshot_input, condition_group, input_delta_path)
