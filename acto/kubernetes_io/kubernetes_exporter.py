...
class KubernetesExporter:

    def __init__(self, namespace: str, kubectl_client: Optional[KubectlClient], crd_meta_info=None,
                 kubeconfig: Optional[str] = None, context_name: Optional[str] = None):
        self.namespace = namespace
        self.crd_meta_info: Optional[dict] = crd_meta_info
        self.log_length = 0

        assert kubectl_client or (
                    kubeconfig and context_name), "Must provide either kubectl client or (kubeconfig, context_name)"

        if kubectl_client:
            apiclient = kubectl_client.init_kube_api_client()
        else:
            apiclient = kubernetes.config.kube_config.new_client_from_config(config_file=kubeconfig,
    ...
    def __get_custom_resources(self, namespace: str, group: str, version: str, plural: str) -> dict:
        ...
        """
        result_dict = {}
        custom_resources = self.customObjectApi.list_namespaced_custom_object(group, version, namespace, plural)[
            'items']
