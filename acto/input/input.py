...
class InputModel:
    ...
    def set_worker_id(self, id: int):
        ...
        self.thread_vars.overspecified_test_plan = TestPlan(
            self.root_schema.to_tree())
        self.thread_vars.copiedover_test_plan = TestPlan(
            self.root_schema.to_tree())
    ...
    def generate_test_plan(
        ...
        logger.info(
            "Parsed [%d] fields from normal schema" %
            len(normal_schemas))
        logger.info(
            "Parsed [%d] fields from over-specified schema" %
            len(pruned_by_overspecified))
        logger.info(
            "Parsed [%d] fields from copied-over schema" %
            len(pruned_by_copied))

        logger.info(
            "Generated [%d] test cases for normal schemas",
        ...
        assert (sum(len(p) for p in self.normal_test_plan_partitioned)
                == len(normal_test_plan_items))
        assert (
            sum(len(p) for p in self.overspecified_test_plan_partitioned) ==
            len(overspecified_test_plan_items))
        assert (
            sum(len(p) for p in self.copiedover_test_plan_partitioned) ==
            len(copiedover_test_plan_items))
    ...
    def next_test(self) -> List[Tuple[TestGroup, TestCase]]:
        ...
        logger.info("Progress [%d] cases left" %
        ...
        for selected_field in selected_fields:
            ...
            ret.append(
                tuple(
                    [selected_field,
                     selected_field.get_next_testcase()]))
...
class DeterministicInputModel(InputModel):
    ...
    def generate_test_plan(
        ...
        for schema in normal_schemas:
            ...
            if isinstance(schema, known_schemas.K8sSchema):
                ...
                for semantic_testcase in semantic_testcases_:
                    if isinstance(
                            semantic_testcase,
                            known_schemas.K8sInvalidTestCase):
                        num_total_invalid_tests += 1
                    num_total_semantic_tests += 1

            if not isinstance(
                schema, known_schemas.K8sSchema) and not covered_by_k8s(
                self.k8s_paths, list(
                    schema.path)):
        ...
        copiedover_test_plan_items = list(copiedover_testcases.items())
        semantic_test_plan_items = list(semantic_testcases.items())
        additional_semantic_test_plan_items = list(
            additional_semantic_testcases.items())
        # randomize to reduce skewness among workers
        random.shuffle(normal_test_plan_items)
        random.shuffle(overspecified_test_plan_items)
        random.shuffle(copiedover_test_plan_items)
        ...
        def split_into_subgroups(
                test_plan_items) -> List[List[Tuple[str, List[TestCase]]]]:
            all_testcases = []
        ...
        additional_semantic_testcases = split_into_subgroups(
            additional_semantic_testcases.items())
...
if __name__ == "__main__":
    ...
    input_model: InputModel = InputModel(
        context["crd"]["body"], [], config.example_dir, 1, 1, [])
    if args.blackbox:
        pruned_list = []
